    ...
    private void resolveIntent(Intent intent) {
    	...
    	if(flag==0)
    	{
			...
			if (NfcAdapter.ACTION_TECH_DISCOVERED.equals(action)) {
				...
				mydialog = ProgressDialog.show(CopyActivity.this, "loading","loading");
			...
			if (NfcAdapter.ACTION_TECH_DISCOVERED.equals(action)) {
				...
				try { // 5.1) Connect to card
					...
					for (int j = 0; j < secCount; j++) {
						...
						boolean aauth = mfc.authenticateSectorWithKeyA(j,Converter.hexStringToByteArray("FFFFFFFFFFFF"));
						boolean bauth = mfc.authenticateSectorWithKeyB(j,Converter.hexStringToByteArray("FFFFFFFFFFFF"));
						...
						if (auth) {
							...
							for (int i = 0; i < bCount; i++) {
								...
								byte []data = mifareClassCard.getSector(j).blocks[i].getData();
								mfc.authenticateSectorWithKeyA(j,Converter.hexStringToByteArray("FFFFFFFFFFFF"));
								mfc.authenticateSectorWithKeyB(j,Converter.hexStringToByteArray("FFFFFFFFFFFF"));
								mfc.writeBlock(j*4+i, data);
								bIndex++;
							}
						} else { // Authentication failed - Handle it
